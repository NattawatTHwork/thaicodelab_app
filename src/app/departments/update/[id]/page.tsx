...
const DepartmentUpdate = ({ params }: { params: { id: string } }) => {
  ...
  const fetchData = async () => {
    if (session?.user?.token) {
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/department/${id}`, {
          method: "GET",
          headers: {
        ...
        if (response.ok && result.status) {
          const mappedFormData = {
            department_code: result.data.department_code,
  ...
  return (
    <DefaultLayout>
      ...
      <Breadcrumb pageName={["User Management", "Departments", "Department Update"]} />
