                ...
                <div className="overflow-x-auto sm:overflow-visible">
                  <table className="min-w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead className="text-xs text-gray-700 uppercase bg-gray-100 dark:bg-gray-700 dark:text-gray-300">
                      <tr>
                        {["rank_code", "full_rank", "short_rank", "sequence"].map((key) => (
                          <th
                            key={key}
                          ...
                          <td className="px-6 py-4 text-center relative">
                            ...
                            {dropdownOpen === rank.rank_id && (
                              <div className="absolute right-0 top-full mt-2 w-40 bg-white border rounded shadow-md z-10 whitespace-nowrap">
                                {userPermissions.includes(4) && <Link href={`/ranks/detail/${rank.rank_id}`}><button className="block w-full px-4 py-2 text-left hover:bg-gray-200">View</button></Link>}
                                {userPermissions.includes(5) && <Link href={`/ranks/update/${rank.rank_id}`}><button className="block w-full px-4 py-2 text-left hover:bg-gray-200">Update</button></Link>}
                                {userPermissions.includes(8) && <button onClick={() => handleDelete(rank.rank_id, rank.rank_code)} className="block w-full px-4 py-2 text-left text-red-600 hover:bg-gray-200">Delete</button>}
                              </div>
                            )}
                          </td>
                        </tr>
