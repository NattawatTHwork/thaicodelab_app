...
const EquipmentTransactionStatusByEquipment = ({ params }: { params: { id: string } }) => {
    ...
    const fetchData = async () => {
        if (session?.user?.token) {
            try {
                const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/equipmenttransaction/details-with-transaction-by-equipment/${id}`, {
                    method: "GET",
                    headers: {
    ...
    return (
        <DefaultLayout>
            <div className="mx-auto max-w-270">
                <Breadcrumb pageName={["Equipment Management", "Equipment Transactions", "Transaction By Equipment"]} />
...
export default EquipmentTransactionStatusByEquipment;
